import React from 'react';
//import Logo from './WhatsAppr.png';
import './Footer.css';

const Footer = () => {
    return(
    <div className="Footer">
        <p>Made with ❤️ by <a href="https://recurshawn.github.io/">Shawn Pereira</a> Twitter:<a href="https://twitter.com/Incept_shawn">@Incept_shawn</a>
        <br/><a target="_blank" rel="noopener noreferrer" href="https://ko-fi.com/recurshawn">🚀 Support this project!</a></p>
        Disclaimer: WhatsAppr is not affiliated with WhatsApp Inc.
        <br/>2020 © WhatsAppr | All rights reserved
    </div>
    );
}

export default Footer;